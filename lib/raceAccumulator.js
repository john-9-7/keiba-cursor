...
function getMergedRecent(maxSnapshots = 100) {
  ...
  return snaps.map((s) => {
    ...
    const buyHitFirst =
      ...
      verdict === '買い' &&
      res.first != null &&
      suggestedHorseNumbers.includes(Number(res.first));
...
function computeStatsFromMerged(merged) {
  ...
  /** @type {Record<string, { snapshots: number, withResult: number, buy: number, skip: number, buyWithResult: number, bbTopFirstWhenBuy: number, buyHitFirst: number, hitRatePercent: number, firstHist: Record<string, number>, secondHist: Record<string, number>, thirdHist: Record<string, number> }>} */
  const byRpt = {};
  ...
  for (const row of merged) {
    ...
    if (row.result) {
      ...
      bump(b.firstHist, row.firstBbRank);
      bump(b.secondHist, row.secondBbRank);
      bump(b.thirdHist, row.thirdBbRank);
      if (row.verdict === '買い') {
        buyWithResult++;
        b.buyWithResult++;
        if (row.buyHitFirst === true) b.buyHitFirst++;
