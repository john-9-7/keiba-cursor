...
/** 回収率に使う5券種のうち、いずれかの払戻行が netkeiba から入っているか */
const ROI_PAYOUT_KEYS = ['umaren', 'wide', 'umatan', 'sanrenpuku', 'sanrentan'];

function hasRoiPayoutPayload(result) {
  if (!result || !result.payouts || typeof result.payouts !== 'object') return false;
  return ROI_PAYOUT_KEYS.some(
    (k) => Array.isArray(result.payouts[k]) && result.payouts[k].length > 0,
  );
}
...
function evaluateReturns(result, plan) {
  ...
  const hasPayouts = hasRoiPayoutPayload(result);
...
function computeStatsFromMerged(merged) {
  ...
  return {
    ...
    totalByType,
    legend:
      '「条件的中」= 買い判定レースで、実際に生成した買い目（馬連/ワイド/馬単/3連複/3連単）のいずれかが的中した件数。「回収率」= 各買い目100円固定での 払戻/投資。上部の「結果が紐づいた件数」と「買い判定かつ結果あり」の差は、多くの場合<strong>見送り</strong>レースに結果が付いているためです。',
    byRpt,
  };
}

/**
 * 結果・払戻の欠損を一覧（集計と同じ merged 範囲）
 * @param {ReturnType<typeof getMergedRecent>} merged
 */
function computeDataGapsFromMerged(merged) {
  const rows = [];
  let missingResult = 0;
  let missingPayout = 0;
  let missingPayoutBuy = 0;
  let skipWithResult = 0;

  for (const row of merged) {
    const verdict = row.verdict || '';
    const hasRes =
      row.result &&
      row.result.first != null &&
      row.result.second != null &&
      !Number.isNaN(Number(row.result.first)) &&
      !Number.isNaN(Number(row.result.second));

    if (verdict === '見送り' && hasRes) skipWithResult += 1;
    ...
    if (!hasRes) {
      ...
      rows.push({
        ...
        venue: row.venue || '',
        raceIndex: row.raceIndex != null ? row.raceIndex : null,
        raceId: row.raceId,
        verdict,
        rpt: row.rpt,
        issue: '結果なし',
        hint: '「結果を自動取得」で開催日を指定',
        first: null,
        second: null,
        third: null,
      });
    ...
    if (!hasRoiPayoutPayload(row.result)) {
      missingPayout += 1;
      if (verdict === '買い') missingPayoutBuy += 1;
      rows.push({
        meetingDate: row.meetingDate || '',
        venue: row.venue || '',
        raceIndex: row.raceIndex != null ? row.raceIndex : null,
        ...
        hint:
          verdict === '買い'
            ? '「払戻だけ再取得」または期間一括（回収率に必要）'
            : '任意。補完しても回収率集計には通常使いません',
        first: row.result.first,
        second: row.result.second,
        third: row.result.third,
      });
    }
  }

  rows.sort((a, b) => {
    const da = String(a.meetingDate || '').replace(/\D/g, '');
    const db = String(b.meetingDate || '').replace(/\D/g, '');
    if (da !== db) return da.localeCompare(db);
    const va = String(a.venue || '').localeCompare(String(b.venue || ''), 'ja');
    if (va !== 0) return va;
    return (Number(a.raceIndex) || 0) - (Number(b.raceIndex) || 0);
  });
