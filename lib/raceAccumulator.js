...
function computeStatsFromMerged(merged) {
  ...
  /** @type {Record<string, { snapshots: number, withResult: number, buy: number, skip: number, buyWithResult: number, bbTopFirstWhenBuy: number, buyHitFirst: number, hitRatePercent: number }>} */
  const byRpt = {};
  ...
  return {
    ...
    bbTopFirstWhenBuy,
    legend:
      '「条件的中」= 買い判定レースで、実際に生成した買い目（馬連/ワイド/馬単/3連複/3連単）のいずれかが的中した件数。',
    byRpt,
  };
}
