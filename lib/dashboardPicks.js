 ...
 * @param {string | null} timeStr "13:25" など
 * @param {number} [nowMs] 現在時刻 UTC ms（省略時は Date.now()）
 */
function isPostTimePassed(dateNorm, timeStr, nowMs = Date.now()) {
  const ms = startTimeToUtcMs(dateNorm, timeStr || '');
