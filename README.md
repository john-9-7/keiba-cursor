...
- **[操作手順（操作手順.md）](docs/操作手順.md)** … **本番サーバー**での使い方（URL・環境変数・取得・トラブル）。**スマホから本番で蓄積を残す**手順は「7. 蓄積データの場所」を参照
- **`render.yaml`** … Render で **永続ディスク + `KEIBA_DATA_DIR`** を付ける Blueprint 例（データが再起動で消えないようにする）
