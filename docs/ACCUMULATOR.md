...
無料Webサービスではディスクが**再起動で消える**ことが多いです。**有料にせず**データを残したい場合は、**手元PCで `npm start`＋トンネルでスマホからアクセス**するか、**`/accumulate-view.html` の CSV を定期的に保存**してください。詳細は **`docs/操作手順.md` §7**。リポジトリの **`render.yaml`** は永続ディスクなしの無料向け例です。
